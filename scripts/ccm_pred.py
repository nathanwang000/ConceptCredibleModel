'''
train file for ccm prediction model
This is inspired by unsupervised domain transfer

x->r: extract the intermediate representation; this link also make l(r, pred(c)) large
c->r: predict the intermediate representation from c
(r, c)->y: predict y based on all representation

Implementation wise use 2 optimizers
a) custom model
b) custom train_step (take 2 optimizers)

see how my mtl is implemented for an inspiration
'''
import sys, os
import tqdm
import numpy as np
