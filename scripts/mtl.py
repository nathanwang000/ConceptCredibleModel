...
from lib.models import MLP, MTL
...
def get_args():
    ...
    parser.add_argument("--c_model_path", type=str,
                        default="outputs/9843d41ae4c711ebb773ac1f6b24a434/concepts",
...
def multi_task(flags, loader_xy, loader_xy_eval, loader_xy_te, loader_xy_val=None,
    ...
    '''
    c_net = torch.load(flags.c_model_path + ".pt") # concept model
    
    # regular model
    net_base = torch.hub.load('pytorch/vision:v0.9.0',
                              'inception_v3', pretrained=True)
    net_base.fc = nn.Identity() 
    net_base.aux_logits = False
    # net.AuxLogits.fc = nn.Linear(768, 200)

    net = MTL(nn.Sequential(c_net, nn.Sigmoid()),
              net_base,
              nn.Linear(2048, 108), nn.Linear(2048, 200))
    net.to(device)

    def criterion(o, y):
        return F.cross_entropy(o[0], y) + F.binary_cross_entropy_with_logits(o[1], o[2])
...
if __name__ == '__main__':
    flags = get_args()
    model_name = f"{RootPath}/{flags.outputs_dir}/mtl"
    print(model_name)
    ...
    run_train = lambda **kwargs: multi_task(
